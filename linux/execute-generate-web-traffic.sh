...
rm -rf /generate-web-traffic
mkdir -p /generate-web-traffic
cd /generate-web-traffic
log "Enumerating urls..."
START_HASH=$(sha256sum --text /tmp/payload_$SCRIPTNAME | awk '{ print $1 }')
while true; do
    log "Running url enumeration:\ncurl -s --retry 20 --retry-connrefused --retry-delay 60 --connect-timeout 5 'urls'; log "curl urls result: $?""
    curl -s --retry 20 --retry-connrefused --retry-delay 60 --connect-timeout 5 'urls'; log "curl urls result: $?"
