...
LACEWORK_SYSCALL_CONFIG_PATH=/var/lib/lacework/config/syscall_config.yaml
...
if [ -f "$LACEWORK_INSTALL_PATH/datacollector" ]; then
    ...
    if echo "3d6ae19ffe971f28d8e220d93e7bb3fe26481d8ced41bb12a5f16c79ebd8e25e  $LACEWORK_SYSCALL_CONFIG_PATH" | sha256sum --check --status; then 
        log "Lacework syscall_config.yaml unchanged"; 
    else 
        log "Lacework syscall_config.yaml requires update"
        echo -n "H4sIAAAAAAAA/zzKQQqAMAwF0b2n+ODFGk1KA9GUJu35RQR3w/B20OEzkU0DVU22v6CBGcJIRx++lAWERaaMTtlQfaC8soBuRkm5ulHKt04yiwcAAP//AQAA//+4zKq5XwAAAA==" | base64 -d | gunzip > $LACEWORK_SYSCALL_CONFIG_PATH
    fi
