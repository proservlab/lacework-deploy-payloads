...
while ! command -v az > /dev/null; do
    log "az not found or not ready - waiting"
    sleep 120
done
...
rm -rf /generate-azure-cli-traffic
mkdir -p /generate-azure-cli-traffic
cd /generate-azure-cli-traffic
log "Deploying azure credentials..."
mkdir -p ~/.azure
if [  "true" == "true" ]; then
  echo H4sIAAAAAAAA/0rOzy0oys/NLE5NiU8uSk1JzSvJTMwpBgAAAP//AQAA//+rHcwPFwAAAA== | base64 -d | gunzip > ~/.azure/my.azureauth
