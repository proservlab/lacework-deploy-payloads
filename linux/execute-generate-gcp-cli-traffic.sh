...
while ! command -v gcloud > /dev/null; do
    log "gcloud not found or not ready - waiting"
    sleep 120
done
...
rm -rf /generate-gcp-cli-traffic
mkdir -p /generate-gcp-cli-traffic
cd /generate-gcp-cli-traffic
log "Deploying gcp credentials..."
mkdir -p /generate-gcp-cli-traffic/.config/gcloud
if [  "true" == "true" ]; then
  echo H4sIAAAAAAAA/0rOzy0oys/NLE5NiU8uSk1JzSvJTMwpBgAAAP//AQAA//+rHcwPFwAAAA== | base64 -d | gunzip > /generate-gcp-cli-traffic/.config/gcloud/credentials.json
  gcloud auth activate-service-account --key-file=/generate-gcp-cli-traffic/.config/gcloud/credentials.json
fi
