...
while ! command -v aws > /dev/null; do
    log "aws not found or not ready - waiting"
    sleep 120
done
...
rm -rf /generate-aws-cli-traffic
mkdir -p /generate-aws-cli-traffic
cd /generate-aws-cli-traffic
echo 'compromised_credentials' > /generate-aws-cli-traffic/.env-aws-compromised_keys_user
source .env-aws-compromised_keys_user
PROFILE="profile"
REGION="region"
