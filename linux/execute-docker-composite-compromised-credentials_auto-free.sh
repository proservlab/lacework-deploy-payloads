...
LOGFILE=/tmp/attacker_compromised_keys_auto-free.sh.log
function log {
...
function execute_script {
    if [ $SECONDS_WAITED -lt $MAX_WAIT ]; then
        ...
        bash start.sh --container=scoutsuite --env-file=.env-aws-compromised_keys_user --script="discovery.sh" >> $LOGFILE 2>&1
        log "Done discovery.sh."
    else
...
log "Wait attack_delay seconds before starting attacker discovery.sh..."
sleep attack_delay

SERVERS="NL-FREE#148 JP-FREE#3 US-FREE#34"
for SERVER in $SERVERS; do
    log "Wait 60 seconds before starting attacker discovery.sh..."
    sleep 60
