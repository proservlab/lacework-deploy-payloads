...
while true; do
    log "starting script"
    if [[ `sudo docker ps | grep minergate_name` ]]; then docker stop minergate_name; fi
    sudo docker run --rm -d --network=host --name minergate_name minergate_image -a cryptonight -o minergate_server -u minergate_user -p x
