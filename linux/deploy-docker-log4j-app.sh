...
while true; do
    if [[ `sudo docker ps | grep log4shell` ]]; then docker stop log4shell; fi
    log "$(echo 'docker run -d --name log4shell -v /tmp:/tmp --rm -p listen_port:8080 ghcr.io/christophetd/log4shell-vulnerable-app@sha256:6f88430688108e512f7405ac3c73d47f5c370780b94182854ea2cddc6bd59929')"
    docker run -d --name log4shell -v /tmp:/tmp --rm -p listen_port:8080 ghcr.io/christophetd/log4shell-vulnerable-app@sha256:6f88430688108e512f7405ac3c73d47f5c370780b94182854ea2cddc6bd59929 >> $LOGFILE 2>&1
