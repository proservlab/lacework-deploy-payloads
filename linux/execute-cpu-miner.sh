...
MINERGATE_USER="minergate_user"
MINERGATE_SERVER="minergate_server"
VERSION="xmrig_version"
    
log "starting script"
while true; do
    log "killing screen sessions..."    
    screen -S host_cpu_miner -X quit
    screen -wipe
    log "truncating screen log..."
    truncate -s 0 /tmp/host_cpu_miner.log
    log "removing previous app directory"
    rm -rf /host_cpu_miner
    log "building app directory"
    mkdir -p /host_cpu_miner/templates
    cd /host_cpu_miner
...
EOF
    screen -d -L -Logfile /tmp/host_cpu_miner.log -S host_cpu_miner -m /host_cpu_miner/xmrig-$VERSION/xmrig -c config.json
    screen -S host_cpu_miner -X colon "logfile flush 0^M"
