...
while true; do
    log "scan target: nmap_scan_host 69"
    log "checking for nmap"
    if ! command -v nmap; then
        log "nmap not found"
        log "downloading: https://github.com/andrew-d/static-binaries/blob/master/binaries/linux/x86_64/nmap?raw=true"
        if [ -f /tmp/nmap ]; then
            curl -L -o /tmp/nmap https://github.com/andrew-d/static-binaries/blob/master/binaries/linux/x86_64/nmap?raw=true >> $LOGFILE 2>&1
            chmod 755 /tmp/nmap >> $LOGFILE 2>&1
        fi
        log "using nmap: /tmp/nmap"
        /tmp/nmap -sT -p 69 nmap_scan_host >> $LOGFILE 2>&1
    else
        nmap -sS -p 69 nmap_scan_host >> $LOGFILE 2>&1
    fi
