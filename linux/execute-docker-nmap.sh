...
while true; do
    ...
    log "Targets: targets"
    echo "targets" > /tmp/nmap-targets.txt
    log "Ports: 69"
    if docker ps -a | grep container_name; then 
        docker stop container_name
        docker rm container_name
    fi
    log "Running via docker: nmap -Pn -sS -T2 -oX /tmp/scan.xml -p69 -iL /tmp/nmap-targets.txt"
/bin/bash -c "docker run --rm -v /tmp:/tmp --entrypoint=nmap --name container_name image -Pn -sT -T2 -oX /tmp/scan.xml -p69 -iL /tmp/nmap-targets.txt || true"
/bin/bash -c "docker logs container_name >> $LOGFILE 2>&1"
