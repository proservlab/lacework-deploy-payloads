...
function Restart-Computer-IfNeeded {
  # Example usage:
  if (Get-PendingReboot) {
    Write-Host "A pending reboot was detected. Rebooting now..."
    Restart-Computer -Force
  } else {
...
function Wait-ForPackageManager {
  ...
  while ($waitTime -lt $maxWaitTime) {
      if (-not (Check-ChocolateyInUse)) {
          Write-Log "Package manager is not in use. Proceeding with installation."
          return
      }
      Start-Sleep -Seconds 5
      $waitTime += 5
  }
  Write-Log "Package manager is still in use after $maxWaitTime seconds. Exiting..."
  exit 1
}
